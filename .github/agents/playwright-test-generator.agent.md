...
- Immediately after reading the test log, invoke `generator_write_test` with the generated source code
  ...
  - Save generated specs under `e2e/` using repo-relative paths only
  - Test must be placed in a describe matching the top-level test plan item
  - Test title must match the scenario name
  - Preserve any Test Case ID included in the scenario name/title
  - If the plan references a seed file, use `e2e/seed.spec.ts` unless the plan explicitly names a different `e2e/` seed
